...
func TestSnapshotMissingPartial(t *testing.T) {
	...
	if st != "PARTIAL" {
		t.Fatalf("expected PARTIAL, got %s", st)
	}
}

func TestConsentTamperFails(t *testing.T) {
	opts := policylock.SnapshotOptions{CreatedAtUTC: "2026-01-01T00:00:00Z", ToolVersion: "policyguardian/v0.1.0-test", UserAgent: "policyguardian/v0.1.0-test"}
	zipb, _, err := policylock.SnapshotFromFile("../../fixtures/policylock/policy1.txt", opts)
	if err != nil {
		t.Fatal(err)
	}
	tmp := t.TempDir() + "/snap.zip"
	if err := os.WriteFile(tmp, zipb, 0644); err != nil {
		t.Fatal(err)
	}
